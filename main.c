...
void Auto_Stack(int* ConeCount_Ptr, int Direction) {
	...
	if(Direction == 1) {
		...
		while(SensorValue[Intake_Pot] < 3300) {
				if(SensorValue[Intake_Pot] > 2500) { Set_Intake(100); Set_Lift(20); }
				Set_Chain_Bar(100);
		} // while(SensorValue[Intake_Pot] < 3300) {

		// Set intake,chainbar passive
		Set_Chain_Bar(0);
...
task Auto_Lift(){
	// Set up local variables
	int Lift_Power_Sign;
	int Lift_Down = 0;		// is 1 if either of the in swithces are pressed, 0 otherwise

	while(true) {
		// Update Lift_Down (needed for passive power)
		Lift_Down = SensorValue[Lift_Down];

		if(Lift_Enable == 1) {
			//// Lift setup (setup once enabled, but prior to robot moving) ////

			// Determine sign of power
			Lift_Power_Sign = (Lift_Power > 0) - (Lift_Power < 0);

			// Set Lift to Power (get mogo moving)
			Set_Lift(Lift_Power);

			// Loop while Lift is enabled
			while(Lift_Enable == 1) {
				// Update Mogo_In, Mogo_Out
				Lift_Down = SensorValue[Lift_Down];

				if(Lift_Power_Sign == 1 && Lift_Down == 1) {
					Set_Lift(10);
					Lift_Enable = 0;
					break;
				} // if(Lift_Power_Sign == 1 && Lift_Down == 1) {
...
void Intake(int Power) {
	...
	// Enable auto_Intake
	Intake_Enable = 1;
} // task Intake(int Power) {

task Auto_Intake(){
	// Set up local variables
	int Intake_Power_Sign;
	int Intake_In = 0;		// is 1 if either of the in swithces are pressed, 0 otherwise
	int Intake_Out = 0;		// is 1 if either of the out switches are pressed, 0 otherwise

	while(true) {
		// Update Intake_In and Intake_Out (needed for passive power)
		Intake_In = SensorValue[Intake_In_Switch];
		Intake_Out = SensorValue[Intake_Out_Switch];

		if(Intake_Enable == 1) {
			//// Intake setup (setup once enabled, but prior to robot moving) ////

			// Determine sign of power
			Intake_Power_Sign = (Intake_Power > 0) - (Intake_Power < 0);

			// Set intake to Power (get intake moving)
			Set_Intake(Intake_Power);

			// Loop while intake is enabled
			while(Intake_Enable == 1) {
				// Update Intake_In, Intake_Out
				Intake_In = SensorValue[Intake_In_Switch];
				Intake_Out = SensorValue[Intake_Out_Switch];

				if(Intake_Power_Sign == 1 && Intake_In == 1) {
					Set_Intake(10);
					Intake_Enable = 0;
					break;
				} // if(Intake_Power_Sign == 1 && Intake_In == 1) {
				else if(Intake_Power_Sign == -1 && Intake_Out == 1) {
					Set_Intake(-10);
					Intake_Enable = 0;
					break;
				} // else if(Intake_Power_Sign == -1 && Intake_Out == 1) {
...
task autonomous() {
	...
	*/
	...
	TurnPID.Offset = 0;	// We've backed up against the wall, all errors should be reset
	DrivePID.Offset = 0;
	...
	clearTimer(T1);
	timer = time1(T1);
	while(SensorValue[Lift_Down_Switch] == 0 && timer < 1500) {
		timer = time1(T1);
		Set_Lift(100);
	}
	Set_Lift(-10);
